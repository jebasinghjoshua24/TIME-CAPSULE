...
function getActionButtons(userId, relationship) {
    switch(relationship.status) {
        case 'friend':
            return `
                <button class="userbase-btn userbase-btn-danger" onclick="ub_removeFriend('${userId}')">
        ...
        case 'pending':
            return `
                <button class="userbase-btn userbase-btn-warning" onclick="ub_cancelFriendRequest('${relationship.requestId}')">
        ...
        case 'received':
            return `
                <button class="userbase-btn userbase-btn-success" onclick="ub_acceptFriendRequest('${relationship.requestId}')">
                    <i class="fas fa-check"></i> Accept
                </button>
                <button class="userbase-btn userbase-btn-danger" onclick="ub_declineFriendRequest('${relationship.requestId}')">
        ...
        default:
            return `
                <button class="userbase-btn userbase-btn-primary" onclick="ub_sendFriendRequest('${userId}', '${getUsername(userId)}')">
...
// ==================== Send Friend Request ====================
function ub_sendFriendRequest(userId, username) {
...
function ub_acceptFriendRequest(requestId) {
...
function ub_declineFriendRequest(requestId) {
...
function ub_cancelFriendRequest(requestId) {
    const index = friendRequests.findIndex(r => r.id === requestId);
    if (index !== -1) {
        friendRequests.splice(index, 1);
...
// ==================== Remove Friend ====================
function ub_removeFriend(userId) {
...
function setupEventListeners() {
    ...
    if (searchInput) {
        searchInput.addEventListener('input', () => {
            ...
            searchTimeout = setTimeout(ub_applyFilters, 300);
        });
    }
...
// Export functions
window.ub_sendFriendRequest = ub_sendFriendRequest;
window.ub_acceptFriendRequest = ub_acceptFriendRequest;
window.ub_declineFriendRequest = ub_declineFriendRequest;
window.ub_cancelFriendRequest = ub_cancelFriendRequest;
window.ub_removeFriend = ub_removeFriend;
