...
function openModal(modalElement, options = {}) {
    ...
    // Set size styles directly
    let width = '800px';
    if (size === 'small') width = '400px';
    if (size === 'medium') width = '600px';
    if (size === 'large') width = '800px';
    if (size === 'xlarge') width = '1000px';
    if (size === 'xxlarge') width = '1200px';
...
function handleOutsideClick(e) {
    if (activeModal && activeModal.options.closeOnClickOutside) {
        ...
        if (e.target === activeModal.element) {
            closeModal();
        }
    }
}
...
function createModal(content, title = '', options = {}) {
    ...
    modalContent.style.cssText = `
        background: var(--dark-card-bg, #1e1e36);
        border-radius: 30px;
        padding: ${options.modalClass === 'admin-panel-modal' ? '0' : '2rem'};
        border: 2px solid var(--dark-border, #2d2d4a);
        width: 90%;
        max-width: ${width};
        max-height: 90vh;
        overflow-y: auto;
    ...
    if (title) {
        ...
        titleEl.style.cssText = `
            ...
            text-align: center;
            color: var(--dark-accent-primary, #6c5ce7);
    ...
    if (showCloseButton) {
        ...
        closeBtn.style.cssText = `
            color: var(--dark-text-muted, #6b6b8b);
            position: absolute;
            top: 1rem;
            right: 1.5rem;
            font-size: 2rem;
        ...
        closeBtn.onmouseout = () => closeBtn.style.color = 'var(--dark-text-muted, #6b6b8b)';
        closeBtn.onclick = (e) => {
            e.preventDefault();
            e.stopPropagation();
            closeModal();
